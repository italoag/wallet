 ...
 * 
 * <p>Verifies that trace context propagates correctly through Project Reactor
...
class ReactiveTracingIntegrationTest extends BaseIntegrationTest {
    ...
    void shouldPreserveTraceContextAcrossSchedulers() {
        ...
        // Note: This depends on implementation details of when scheduler transitions are detected
    }
    ...
    void shouldTraceRedisReactiveOperations() {
        ...
        // Verify Redis spans
        List<FinishedSpan> redisSpans = findSpans(spans, s -> 
            s.getName() != null && s.getName().startsWith("cache."));
    ...
    void shouldTraceRedisMultiGetOperation() {
        ...
        List<FinishedSpan> spans = getSpans();
        FinishedSpan multiGetSpan = findSpan(spans, s -> 
            s.getName() != null && s.getName().equals("cache.mget"));
