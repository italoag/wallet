 ...
 * A transaction involves transferring value from one address to another within a specific network.
 * It encapsulates essential details such as transaction hash, sender and recipient addresses, value,
 * gas details, and the transaction's status within the blockchain lifecycle.
 *
 * A transaction starts as PENDING and can be CONFIRMED or FAILED based on the blockchain processing result.
 * This class extends the AggregateRoot, enabling it to track domain events associated with the transaction state changes.
 */
@Getter
public class Transaction extends AggregateRoot {
   ...
   */
    public static Transaction create(
        ...
        transaction.registerEvent(new TransactionCreatedEvent(id, networkId, hash.getValue(), fromAddress, toAddress, null));
        return transaction;
    }
  ...
  /**
   * Confirms the transaction by updating its status to CONFIRMED and registering a
   * TransactionStatusChangedEvent. Additionally, updates the block number, block hash,
