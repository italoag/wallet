 ...
 * the wallet repository to fetch and update wallet data, and the domain event
 * publisher
...
public class WithdrawFundsUseCase {
  ...
  /**
   * Withdraws a specified amount of funds from a wallet, updates the wallet's
   * balance in the repository,
   * and publishes a FundsWithdrawnEvent.
   *
   * @param walletId      the unique identifier of the wallet from which funds are
   *                      to be withdrawn
   * @param amount        the amount to withdraw from the wallet; must be greater
   *                      than zero and less than or equal to the wallet's current
   *                      balance
   * @param correlationId the unique identifier used to correlate this withdrawal
   *                      operation, typically for tracking purposes
   * @throws IllegalArgumentException if the wallet cannot be found or the amount
   *                                  is invalid or exceeds the wallet's available
   *                                  balance
   */
