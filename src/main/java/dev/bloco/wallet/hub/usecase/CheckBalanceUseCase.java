...
/**
 * CheckBalanceUseCase is responsible for retrieving the current balance of a
 * specific wallet.
 * It encapsulates the logic for identifying a wallet by its unique identifier
 * and obtaining
 ...
 * <p/>
 * Uses:
 * - WalletRepository for fetching the Wallet entity from the system's
 * persistent storage.
 * <p/>
 * Exceptions:
 * - IllegalArgumentException is thrown if the wallet cannot be found in the
 * repository.
 */
...
public class CheckBalanceUseCase {
   ...
   *
   * @param walletId the unique identifier of the wallet whose balance is to be
   *                 retrieved
   * @return the current balance of the wallet as a BigDecimal
   * @throws IllegalArgumentException if the wallet with the specified ID does not
   *                                  exist
   */
