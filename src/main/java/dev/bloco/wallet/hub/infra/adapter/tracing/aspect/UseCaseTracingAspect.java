...
public class UseCaseTracingAspect {
    ...
    public Object traceUseCaseExecution(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        return observation.observe(() -> {
            try {
                ...
                // observation.lowCardinalityKeyValue("status", "error"); // Builder handles
                // this or redundant
    ...
    private void addUseCaseAttributes(Observation observation, ProceedingJoinPoint joinPoint, String className) {
        try {
            ...
            UseCaseAttributes attributes = extractAttributes(joinPoint, args);
            ...
            if (attributes.userHash != null) {
                observation.highCardinalityKeyValue("user.id.hash", attributes.userHash);
            }
    ...
    private UseCaseAttributes extractAttributes(ProceedingJoinPoint joinPoint, Object[] args) {
        UseCaseAttributes attributes = new UseCaseAttributes();
        if (args == null) {
            return attributes;
        }
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            String stringValue = arg.toString();

            if (paramName.toLowerCase().contains("transaction")) {
                attributes.transactionId = stringValue;
            } else if (paramName.toLowerCase().contains("wallet")) {
                attributes.walletId = hashIdentifier(stringValue);
            } else if (paramName.toLowerCase().contains("user")) {
                attributes.userHash = hashIdentifier(stringValue);
            } else if (paramName.toLowerCase().contains("amount")) {
                try {
                    attributes.amount = Double.valueOf(stringValue);
                } catch (NumberFormatException _) {
                    // Ignore invalid amounts
                }
            } else if (paramName.toLowerCase().contains("currency")) {
                attributes.currency = stringValue;
            }
        }
        return attributes;
    }

    private static class UseCaseAttributes {
        String walletId;
