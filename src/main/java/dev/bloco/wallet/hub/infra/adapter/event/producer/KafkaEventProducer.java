...
/**
 * KafkaEventProducer is responsible for producing domain events related to
 * wallet operations
 * and sending them to Kafka through spring-cloud StreamBridge with W3C Trace
 * Context propagation.
 *
 * <p>
 * Events are stored in an outbox for reliable delivery and subsequently sent in
 * batches
 * with trace context injected as CloudEvents 1.0 extensions.
 * </p>
 *
 * <p>
...
public class KafkaEventProducer implements EventProducer {
     ...
     *
     * <p>
     * This constructor initializes the KafkaEventProducer with the necessary
     * dependencies:
     * </p>
     * <ul>
     ...
     * </ul>
     *
     * @param outboxService   the service responsible for storing and retrieving
     *                        events from the outbox
     * @param streamBridge    the Spring Cloud Stream bridge used for sending
     *                        CloudEvents to Kafka
     * @param objectMapper    the Jackson object mapper used for converting event
     *                        objects to JSON
     * @param tracePropagator the CloudEvent trace propagator for W3C Trace Context
     *                        injection
     */
    public KafkaEventProducer(OutboxService outboxService, StreamBridge streamBridge,
        ...
        // In Jackson 3, ObjectMapper is immutable and Spring Boot auto-configures
        // modules via SPI
    ...
    /**
     * Produces a wallet-created event, persisting it into the outbox for further
     * processing.
     * <p/>
     * This method is responsible for saving a wallet creation event with the
     * appropriate
     * event type. The event is serialized and stored in the outbox for eventual
     * delivery
    ...
    /**
     * Produces an event indicating that funds have been added to a wallet and
     * persists it into the outbox.
     * <p/>
     * This method is responsible for saving a funds-added event with the
     * appropriate event type.
     * The event is serialized and stored in the outbox for eventual delivery to
     * external systems or services.
     *
     * @param event the funds-added event containing information about the wallet,
     *              the amount added, and the correlation ID
     */
    @Override
     ...
     *
     * @param event the funds-withdrawn event containing information about the
     *              wallet,
    ...
    private String extractCorrelationId(Object event) {
        try {
            if (event instanceof dev.bloco.wallet.hub.domain.event.common.DomainEvent de
                    && de.getCorrelationId() != null) {
    ...
    /**
     * Processes unsent events from the outbox and sends them to Kafka with W3C
     * Trace Context.
     *
     * <p>
     * This method is scheduled to run at a fixed interval of 5 seconds. It:
     * </p>
     * <ol>
     ...
     *
     * <p>
     * The CloudEvent structure includes:
     * </p>
     * <ul>
     ...
     *
     * <p>
     * The event's Kafka channel is determined by appending "-out-0" to the event
     * type.
     * </p>
     *
     * <p>
     ...
     * </p>
     * <ul>
     * <li>{@code outboxService} - fetches unsent events and updates their
     * status</li>
