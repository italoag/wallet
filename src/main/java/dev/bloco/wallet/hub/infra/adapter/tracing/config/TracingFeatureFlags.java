...
/**
 * Configuration properties for runtime control of tracing feature flags per component.
 * 
 * <h2>Purpose</h2>
 * Enables granular control over which components are instrumented for distributed tracing.
 * Feature flags can be toggled at runtime without service restart via Spring Boot Actuator's
 * refresh endpoint, allowing:
 * <ul>
 *   <li>Performance tuning by disabling tracing for high-volume low-value operations</li>
 *   <li>Troubleshooting by selectively enabling tracing for specific components</li>
 *   <li>Gradual rollout of tracing instrumentation in production</li>
 *   <li>Cost optimization by reducing trace data volume to essential components</li>
 * </ul>
 *
 * <h2>Configuration</h2>
 * <pre>{@code
 ...
 * <ol>
 *   <li>Update configuration in application-tracing.yml (or Spring Cloud Config)</li>
 ...
 * </ol>
 *
 * <p><b>Example Refresh Request:</b></p>
 * <pre>{@code
 ...
 *
 * <p><b>1. Conditional Bean Registration (@ConditionalOnProperty):</b></p>
 * <pre>{@code
 * @Aspect
 * @Component
 ...
 * }</pre>
 *
 * <p><b>2. Runtime Checks in Handlers:</b></p>
 * <pre>{@code
 * @Component
 ...
 * <ul>
 *   <li><b>database</b>: Controls JPA and R2DBC query tracing, connection pool monitoring.
 *       Disable to reduce overhead in read-heavy workloads.</li>
 *   <li><b>kafka</b>: Controls Kafka producer/consumer span creation and trace context propagation.
 *       Disable to reduce overhead in high-throughput event processing.</li>
 *   <li><b>stateMachine</b>: Controls Spring Statemachine transition tracing for saga workflows.
 ...
 *       Disable if external dependencies are stable and don't require monitoring.</li>
 *   <li><b>reactive</b>: Controls reactive pipeline tracing (WebFlux, R2DBC, Redis reactive).
 ...
 *       Disable to reduce baseline tracing overhead (not recommended for production).</li>
 * </ul>
 *
 ...
 * <ul>
 *   <li>Keep all flags enabled (true) by default for comprehensive observability</li>
 *   <li>Disable flags selectively in production only after performance profiling</li>
 ...
 *   <li>Monitor trace volume reduction when disabling flags to validate effectiveness</li>
 *   <li>Use Spring Cloud Config for centralized flag management across service instances</li>
 * </ul>
 *
 * <h2>Integration with Sampling</h2>
 * Feature flags control instrumentation (span creation), while sampling controls export:
 * <pre>
 ...
 * <ul>
 *   <li>If configuration property is missing: defaults to true (fail-safe, enable tracing)</li>
 *   <li>If refresh endpoint fails: retains current configuration (no impact on running app)</li>
 *   <li>If invalid boolean value: Spring Boot validation fails fast on startup</li>
 * </ul>
 *
 * <h2>Future Enhancements</h2>
 * Planned improvements:
 * <ul>
 *   <li>Per-operation granularity (e.g., enable only SELECT queries, not INSERT)</li>
 *   <li>Dynamic sampling rates per component (combine flags with sampling config)</li>
...
public class TracingFeatureFlags {
     ...
     * <p>When enabled, instruments:</p>
     * <ul>
     *   <li>Spring Statemachine state transitions via StateMachineObservationHandler</li>
     ...
     * </ul>
     *
     * <p><b>Warning:</b> Disabling this removes visibility into core business logic.
     * Only disable in extreme performance scenarios after thorough profiling.</p>
     *
     * @default true
     */
    ...
    public void logConfiguration() {
        log.info("TracingFeatureFlags configuration: [database={}, kafka={}, stateMachine={}, " +
                        "externalApi={}, reactive={}, useCase={}]",
                database, kafka, stateMachine, externalApi, reactive, useCase);

        // Warn if critical flags are disabled
        if (!useCase) {
            log.warn("Use case tracing is DISABLED - core business operation visibility lost");
        }
        if (!kafka) {
            log.warn("Kafka tracing is DISABLED - event-driven workflow visibility lost");
        }

        // Info about performance optimization
        int disabledCount = 0;
        if (!database) disabledCount++;
        if (!kafka) disabledCount++;
        if (!stateMachine) disabledCount++;
        if (!externalApi) disabledCount++;
        if (!reactive) disabledCount++;
        if (!useCase) disabledCount++;

        if (disabledCount > 0) {
            log.info("Performance optimization: {} tracing components disabled, " +
                            "estimated overhead reduction: ~{} ms per operation",
                    disabledCount, disabledCount * 1.5);
        } else {
    ...
    public String getEnabledComponents() {
        ...
        if (database) enabled.add("database");
        if (kafka) enabled.add("kafka");
        if (stateMachine) enabled.add("stateMachine");
        if (externalApi) enabled.add("externalApi");
        if (reactive) enabled.add("reactive");
        if (useCase) enabled.add("useCase");
    ...
    public String getDisabledComponents() {
        ...
        if (!database) disabled.add("database");
        if (!kafka) disabled.add("kafka");
        if (!stateMachine) disabled.add("stateMachine");
        if (!externalApi) disabled.add("externalApi");
        if (!reactive) disabled.add("reactive");
        if (!useCase) disabled.add("useCase");
