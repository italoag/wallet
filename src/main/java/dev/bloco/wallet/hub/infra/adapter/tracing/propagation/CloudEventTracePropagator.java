...
/**
 * CloudEventTracePropagator handles distributed trace context propagation
 * across Kafka boundaries
 * using CloudEvents extension attributes following the W3C Trace Context
 * specification.
 * 
 * <p>
 * <b>Problem Statement:</b>
 * </p>
 * In event-driven architectures, maintaining trace continuity across
 * asynchronous message boundaries
 ...
 * <li>Debugging distributed transactions requires manual correlation of
 * logs</li>
 * <li>Consumer lag and event processing delays cannot be measured within a
 * single trace</li>
 * </ul>
 * 
 * <p>
 * <b>Solution: W3C Trace Context in CloudEvents</b>
 * </p>
 * This component implements trace context propagation by:
 * <ol>
 * <li><b>Injection (Producer Side)</b>: Captures current trace context and
 * embeds it as CloudEvent
 * extension attributes (traceparent, tracestate) before publishing to
 * Kafka</li>
 * <li><b>Extraction (Consumer Side)</b>: Reads trace context from CloudEvent
 * extensions and
 * restores it in the consumer's tracer, creating child spans linked to the
 * producer span</li>
 * </ol>
 * 
 * <p>
 * <b>W3C Trace Context Format:</b>
 * </p>
 * 
 * <pre>
 ...
 * 
 * <p>
 * <b>CloudEvents Extension Model:</b>
 * </p>
 * CloudEvents extensions are key-value pairs that extend the event envelope
 * without modifying
 * the event payload (domain data). This keeps infrastructure concerns (tracing)
 * separate from
 * business logic:
 * 
 * <pre>{@code
 ...
 * </p>
 * <ul>
 * <li><b>KafkaEventProducer</b>: Calls {@code injectTraceContext()} before
 * sending events</li>
 * <li><b>Event Consumers</b>: Call {@code extractTraceContext()} in functional
 * consumer preamble</li>
 * <li><b>ObservationRegistry</b>: Works with
 * PropagatingSenderTracingObservationHandler</li>
 * <li><b>Micrometer Tracer</b>: Reads/writes trace context via Tracer API</li>
 * </ul>
 * 
 * <p>
 * <b>Usage Example - Producer:</b>
 * </p>
 * 
 * <pre>{@code
 * @Component
 ...
 * 
 * <p>
 * <b>Trace Context Lifecycle:</b>
 * </p>
 * <ol>
 * <li><b>Active Span Check</b>: Verifies current span exists before
 * injection</li>
 * <li><b>Context Extraction</b>: Reads trace-id, span-id, and sampling
 * flags</li>
 ...
 * <li><b>Context Restoration</b>: TraceContext reconstructed from
 * extensions</li>
 * <li><b>Span Activation</b>: New span created with extracted context as
 * parent</li>
 * </ol>
 * 
 * <p>
 ...
 * </p>
 * <ul>
 * <li><b>No Active Span</b>: Returns original CloudEvent unchanged, logs
 * warning</li>
 * <li><b>Missing traceparent</b>: Creates new root trace, logs info</li>
 * <li><b>Invalid traceparent Format</b>: Creates new root trace, logs
 * warning</li>
 * <li><b>Extraction Failure</b>: Does not fail consumer, logs error and
 * continues</li>
 * </ul>
 * 
 * These behaviors ensure tracing infrastructure failures never break
 ...
 * <li>No impact on event payload size (extensions stored separately in Kafka
 * headers)</li>
 * </ul>
 * 
 * <p>
 ...
 * </p>
 * <ul>
 * <li>{@link io.micrometer.tracing.Tracer}: Source of truth for current trace
 * context</li>
 * <li>{@link io.micrometer.tracing.propagation.Propagator}: W3C format
 * serialization</li>
 ...
 * <li><b>CloudEvents v1.0</b>:
 * https://github.com/cloudevents/spec/blob/v1.0/spec.md</li>
 * <li><b>CloudEvents Distributed Tracing</b>:
 * https://github.com/cloudevents/spec/blob/main/cloudevents/extensions/distributed-tracing.md</li>
 * </ul>
 * 
...
public class CloudEventTracePropagator {
    ...
    /**
     * CloudEvents extension attribute name for message send timestamp (epoch
     * millis).
     ...
     * 
     * <p>
     * This method is called by event producers (typically
     * {@link KafkaEventProducer})
     * before publishing events to Kafka. It captures the active span's trace
     * context
     * and embeds it as CloudEvent extensions following the W3C Trace Context
     * specification.
     * </p>
     * 
     * <p>
     ...
     * </p>
     * <ul>
     * <li>If an active span exists, extracts trace-id, span-id, and sampling
     * decision</li>
     * <li>Formats traceparent as: {@code 00-{traceId}-{spanId}-{flags}}</li>
     * <li>Adds tracestate if present in the current context</li>
     * <li>Returns a new CloudEvent with trace extensions (original event
     * unchanged)</li>
     * <li>If no active span, returns the original event unchanged and logs a
     * warning</li>
     * </ul>
     * 
     * <p>
     * <b>Thread Safety:</b>
     * </p>
     * This method is thread-safe. It reads from the tracer's ThreadLocal context
     * and creates
     ...
     * @return a new CloudEvent with trace context extensions, or the original event
     *         if no active span
    ...
    public CloudEvent injectTraceContext(CloudEvent event) {
        ...
        // Note: Brave bridge doesn't expose tracestate directly, so we only inject
        // traceparent
        ...
        CloudEvent enrichedEvent = CloudEventBuilder.from(event)
                .withExtension(TRACEPARENT_EXTENSION,
                        java.util.Objects.requireNonNull(traceparent, "traceparent cannot be null"))
    ...
    /**
     * Extracts trace context from a CloudEvent and creates a new span in the
     * tracer.
     * 
     * <p>
     * This method is called by event consumers before processing the event. It
     * reads
     * the traceparent extension attribute, parses the trace-id and parent-span-id,
     * and
     * creates a new span that is a child of the producer's span.
     * </p>
     * 
     * <p>
     * <b>Behavior:</b>
     ...
     * <li>Creates a new {@link TraceContext} with the extracted trace-id and parent
     * span-id</li>
     * <li>Starts a new span in the tracer with this context (span becomes
     * active)</li>
     * <li>If traceparent is missing or invalid, creates a new root trace and logs a
     * warning</li>
     ...
     * <b>Span Lifecycle:</b>
     * </p>
     * The returned span is started but not finished. The caller (typically an event
     * consumer)
     * must call {@code span.end()} after processing the event to complete the span
     * lifecycle:
     * 
     * <pre>{@code
     * @Bean
     ...
     * <b>Thread Safety:</b>
     * </p>
     * This method is thread-safe and should be called from the consumer's
     * processing thread
     ...
     * @return a new Span with extracted trace context, or a new root span if
     *         extraction fails
     * @throws NullPointerException if event is null
     */
    public Span extractTraceContext(CloudEvent event) {
        ...
        try {
            ...
            // This ensures the new span is a child of the producer's span, continuing the
            // trace
