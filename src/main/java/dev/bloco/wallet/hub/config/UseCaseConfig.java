...
/**
 * The UseCaseConfig class is a Spring configuration class that defines beans
 * for various use cases
 * within the application. Each use case represents a specific business
 * operation related to wallet
 * and transaction management. The class encapsulates the creation and
 * configuration of these use cases,
 * injecting their required dependencies such as repositories and event
 * publishers.
 * <p/>
 * Beans defined in this configuration:
 * <p/>
 * 1. CreateWalletUseCase
 * - Use case for creating a new wallet. It manages the creation of wallet
 * entities and publishes
 * the wallet creation event.
 * <p/>
 * 2. AddFundsUseCase
 * - Use case for adding funds to a wallet. It handles the wallet's balance
 * update, records the transaction,
 * and publishes a fund addition event.
 * <p/>
 * 3. WithdrawFundsUseCase
 * - Use case for withdrawing funds from a wallet. It checks for sufficient
 * balance, updates the wallet's balance,
 * records the transaction, and publishes a fund withdrawal event.
 * <p/>
 * 4. TransferFundsUseCase
 * - Use case for transferring funds between wallets. This use case deducts the
 * specified amount from the source wallet,
 * adds it to the destination wallet, records the transfer transaction, and
 * publishes a fund transfer event.
 * <p/>
 * 5. CheckBalanceUseCase
 * - Use case for checking the balance of a specific wallet. It retrieves the
...
public class UseCaseConfig {
    ...
    /**
     * Defines a Spring Bean for the CreateWalletUseCase. This use case is
     * responsible for creating
     * a new wallet for a user. It manages the persistence of the wallet entity and
     * publishes
     * a wallet creation event.
     *
     * @param walletRepository the repository responsible for managing wallet
     *                         persistence
     * @param eventPublisher   the publisher responsible for publishing domain
     *                         events, such as wallet creation events
    ...
    /**
     * Defines a Spring Bean for the AddFundsUseCase. This use case is responsible
     * for adding funds
     * to a specific wallet within the system. It manages the validation and
     * updating of the wallet's
     * balance and the publication of a domain event representing the operation.
     *
     * @param walletRepository the repository responsible for accessing and managing
     *                         wallet data
     * @param eventPublisher   the publisher responsible for publishing domain
     *                         events, such as FundsAddedEvent
     * @return a configured instance of AddFundsUseCase
     */
    @Bean
    public AddFundsUseCase addFundsUseCase(WalletRepository walletRepository, DomainEventPublisher eventPublisher) {
        return new AddFundsUseCase(walletRepository, eventPublisher);
    }

    /**
     * Defines a Spring Bean for the WithdrawFundsUseCase. This use case handles the
     * process of withdrawing funds from a specific wallet within the system. It
     * manages
     * the validation and update of the wallet's balance and the publication of a
     * domain event
     ...
     *                         wallet data
     * @param eventPublisher   the publisher responsible for publishing domain
     *                         events, such as FundsWithdrawnEvent
     * @return a configured instance of WithdrawFundsUseCase
     */
    @Bean
    public WithdrawFundsUseCase withdrawFundsUseCase(WalletRepository walletRepository,
            DomainEventPublisher eventPublisher) {
        return new WithdrawFundsUseCase(walletRepository, eventPublisher);
    }

    /**
     * Defines a Spring Bean for the TransferFundsUseCase. This use case is
     * responsible for transferring
     * funds between two wallets within the system. It performs validation and
     * updates the balances of
     * the source and destination wallets and publishes a domain event to signal
     * that the transfer has occurred.
     *
     * @param walletRepository the repository responsible for accessing and managing
     *                         wallet data
     * @param eventPublisher   the publisher responsible for publishing domain
     *                         events, such as FundsTransferredEvent
     * @return a configured instance of TransferFundsUseCase
     */
    @Bean
    public TransferFundsUseCase transferFundsUseCase(WalletRepository walletRepository,
            DomainEventPublisher eventPublisher) {
        return new TransferFundsUseCase(walletRepository, eventPublisher);
    }

    /**
     * Defines a Spring Bean for the CheckBalanceUseCase. This use case is
     * responsible for
     * retrieving the balance of a specific wallet within the system. It validates
     * the existence
    ...
    @Bean
    public UpdateWalletUseCase updateWalletUseCase(WalletRepository walletRepository,
            DomainEventPublisher eventPublisher) {
        return new UpdateWalletUseCase(walletRepository, eventPublisher);
    }

    @Bean
    public ActivateWalletUseCase activateWalletUseCase(WalletRepository walletRepository,
            DomainEventPublisher eventPublisher) {
        return new ActivateWalletUseCase(walletRepository, eventPublisher);
    }

    @Bean
    public DeactivateWalletUseCase deactivateWalletUseCase(WalletRepository walletRepository,
            DomainEventPublisher eventPublisher) {
        return new DeactivateWalletUseCase(walletRepository, eventPublisher);
    }

    @Bean
    public DeleteWalletUseCase deleteWalletUseCase(WalletRepository walletRepository,
            DomainEventPublisher eventPublisher) {
    ...
    @Bean
    public GetWalletDetailsUseCase getWalletDetailsUseCase(WalletRepository walletRepository) {
        return new GetWalletDetailsUseCase(walletRepository);
    }

    @Bean
    public RecoverWalletUseCase recoverWalletUseCase(WalletRepository walletRepository,
            DomainEventPublisher eventPublisher) {
    ...
    @Bean
    public GetAddressBalanceUseCase getAddressBalanceUseCase(AddressRepository addressRepository,
            TokenBalanceRepository tokenBalanceRepository) {
        return new GetAddressBalanceUseCase(addressRepository, tokenBalanceRepository);
    }

    @Bean
    public ListAddressesByWalletUseCase listAddressesByWalletUseCase(AddressRepository addressRepository,
            WalletRepository walletRepository) {
        return new ListAddressesByWalletUseCase(addressRepository, walletRepository);
    }

    @Bean
    public UpdateAddressStatusUseCase updateAddressStatusUseCase(AddressRepository addressRepository,
            DomainEventPublisher eventPublisher) {
        return new UpdateAddressStatusUseCase(addressRepository, eventPublisher);
    }

    @Bean
    public ImportAddressUseCase importAddressUseCase(AddressRepository addressRepository,
            WalletRepository walletRepository,
            NetworkRepository networkRepository, DomainEventPublisher eventPublisher,
            ValidateAddressUseCase validateAddressUseCase) {
        return new ImportAddressUseCase(addressRepository, walletRepository, networkRepository, eventPublisher,
    ...
    @Bean
    public AddTokenToWalletUseCase addTokenToWalletUseCase(WalletRepository walletRepository,
    ...
    @Bean
    public RemoveTokenFromWalletUseCase removeTokenFromWalletUseCase(WalletRepository walletRepository,
    ...
    @Bean
    public GetTokenBalanceUseCase getTokenBalanceUseCase(WalletRepository walletRepository,
            TokenRepository tokenRepository,
            TokenBalanceRepository tokenBalanceRepository) {
    ...
    @Bean
    public ListSupportedTokensUseCase listSupportedTokensUseCase(TokenRepository tokenRepository,
    ...
    @Bean
    public AddNetworkUseCase addNetworkUseCase(NetworkRepository networkRepository,
            DomainEventPublisher eventPublisher) {
    ...
    public ChangePasswordUseCase changePasswordUseCase(UserRepository userRepository,
            UserSessionRepository sessionRepository) {
        return new ChangePasswordUseCase(userRepository, sessionRepository);
    }
    ...
    @Bean
    public GetPortfolioSummaryUseCase getPortfolioSummaryUseCase(WalletRepository walletRepository,
            AddressRepository addressRepository,
            TokenBalanceRepository tokenBalanceRepository, TokenRepository tokenRepository) {
        return new GetPortfolioSummaryUseCase(walletRepository, addressRepository, tokenBalanceRepository,
                tokenRepository);
    }
