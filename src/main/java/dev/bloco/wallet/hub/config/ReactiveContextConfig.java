...
/**
 * ReactiveContextConfig enables automatic context propagation through Project
 * Reactor pipelines
 * to maintain trace context across asynchronous and reactive operations without
 * manual intervention.
 * 
 * <p>
 * <b>Problem Statement:</b>
 * </p>
 * Standard ThreadLocal-based context propagation (used by Micrometer Tracing)
 * breaks in reactive
 * pipelines due to:
 * <ul>
 * <li>Asynchronous execution across multiple threads (subscribeOn,
 * publishOn)</li>
 ...
 * </ul>
 * 
 * Without automatic propagation, trace IDs are lost at reactive operator
 * boundaries, resulting in
 * orphaned spans and broken distributed traces.
 * 
 * <p>
 * <b>Solution: Reactor Context Propagation</b>
 * </p>
 * This configuration activates
 * {@link Hooks#enableAutomaticContextPropagation()}, which:
 * <ol>
 * <li>Automatically captures ThreadLocal context (including trace IDs) when
 * entering reactive chains</li>
 * <li>Stores context in Reactor's immutable
 * {@link reactor.util.context.Context}</li>
 * <li>Restores ThreadLocal context when operators execute on different
 * threads</li>
 * <li>Propagates context through all standard reactive operators (map, flatMap,
 * filter, etc.)</li>
 * </ol>
 * 
 * <p>
 ...
 * </p>
 * <ul>
 * <li><b>Activation Point</b>: Static initializer executes once when class
 * loads (before Spring context creation)</li>
 * <li><b>Scope</b>: Global JVM-wide setting (affects all Reactor publishers in
 * the application)</li>
 * <li><b>Performance Impact</b>: Minimal (<1ms overhead per reactive chain
 * initiation)</li>
 * <li><b>Thread Safety</b>: Reactor Context is immutable and thread-safe</li>
 * <li><b>Integration</b>: Works seamlessly with Micrometer Tracing's
 * ThreadLocal-based propagation</li>
 * </ul>
 * 
 * <p>
 * <b>Supported Reactive Components:</b>
 * </p>
 * <ul>
 ...
 * 
 * <p>
 * <b>Context Propagation Flow Example:</b>
 * </p>
 * 
 * <pre>{@code
 * // HTTP Request arrives with trace header (traceparent:
 ...
 *     // Automatic context restoration: Reactor Context → ThreadLocal (trace ID
 *     // available for logging)
 * }
 * }</pre>
 * 
 * In this example, the trace ID remains accessible throughout the reactive
 * chain, even after the
 * thread switch via {@code subscribeOn}. Without this configuration, the trace
 * ID would be lost
 ...
 * <ul>
 * <li>Reactive dependencies present on classpath
 * ({@code reactor.core.publisher.Mono})</li>
 ...
 * <ol>
 * <li>Verify this configuration is loaded (check for "Enabled automatic Reactor
 * context propagation" log)</li>
 * <li>Check Spring Boot version ≥ 3.2 (required for full support)</li>
 * <li>Ensure {@code micrometer-context-propagation} dependency is on
 * classpath</li>
 * <li>Enable Reactor debug mode: {@code Hooks.onOperatorDebug()} to trace
 * context flow</li>
 ...
 * 
 * <p>
 * <b>Alternatives Not Chosen:</b>
 * </p>
 * <ul>
 * <li><b>Manual contextWrite() everywhere</b>: Rejected (error-prone, verbose,
 * high maintenance)</li>
 * <li><b>ThreadLocal with custom scheduler decorators</b>: Rejected (doesn't
 * work with WebFlux)</li>
 * <li><b>MDC-based propagation</b>: Rejected (breaks in async environments, not
 * reactive-aware)</li>
 * </ul>
 * 
...
public class ReactiveContextConfig {
    ...
    static {
        // Enable automatic context propagation globally before Spring context
        // initialization
        ...
        log.debug(
                "Trace context will now propagate automatically through reactive operators (flatMap, map, subscribeOn, publishOn)");
    }
     ...
     * 
     * <p>
     * Spring will instantiate this bean when tracing is enabled, ensuring the
     * static
     * initializer executes early in the application lifecycle. The bean itself
     * provides
     * no runtime functionality but serves as a configuration marker.
     * </p>
     */
